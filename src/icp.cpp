...
ICP_OUT icp(const Eigen::MatrixXd &A, const Eigen::MatrixXd &B, int max_iterations, float tolerance){
    ...
    // Eigen::MatrixXd dst_test = Eigen::MatrixXd::Ones(row, 3);
    ...
    for (int i=0; i<max_iterations; i++){
        ...
        // std::cout << src3d.rows() << ", " << src3d.cols() << std::endl;
        
        // temp = ICP_single_step_cuda(dst.transpose(), dst_chorder_temp, neighbor);
        // dst_chorder_test(Eigen::all, Eigen::all) = dst_chorder_temp.transpose();

        dst_chorder(Eigen::seqN(0,3), Eigen::all) = dst(Eigen::seqN(0,3), neighbor.indices);
        
        // verify(dst_chorder, dst_chorder_test);
        // std::cout << "So far so good, keep going !\n";
        // exit(0);
